...
void main(void) {
  ...
  int ret;
  if (NULL == dev) {
    LOG_ERROR("Failed to get ALIPSU device binding");
    return;
  }
  ...
  while (retry < 1) {
    ...
    if (ret < 0) {
      LOG_ERROR("Failed to read current page: %d", old_page);
      continue;
    }

    ret = ali_psu_mfr_page_store(dev, old_page+1);
    if (ret < 0) {
      LOG_ERROR("Failed to store new page: %d", old_page+1);
      continue;
    }

    ret = ali_psu_mfr_page_show(dev, &old_page);
    if (ret < 0) {
      LOG_ERROR("Read current updated page: %d", old_page);
      continue;
    }

    ret = ali_psu_fw_version_show(dev, ALI_PS_REG_FW_REV, buf, &len);
    if (ret < 0) {
      LOG_ERROR("Failed to show firmware version: %d", ret);
      continue;
    }

    ret = ali_psu_ac_cycle_store(dev);
    if (ret < 0) {
      LOG_ERROR("Failed to store AC cycle: %d", ret);
      continue;
    }
    
    ret = ali_psu_bootloader_str_show(dev, ALI_PS_REG_BOOTLOADER_KEY, buf, &len);
    if (ret < 0) {
      LOG_ERROR("Failed to show bootloader key: %d", ret);
      continue;
    }
    ...
    if (ret < 0) {
      LOG_ERROR("Failed to store bootloader key: %d", ret);
      continue;
    }
    ...
    if (ret < 0) {
      LOG_ERROR("Failed to store bootloader status: %d", ret);
      continue;
    }

    ret = ali_psu_bootloader_hex_show(dev, ALI_PS_REG_BOOTLOADER_STATUS, buf, &len);
    if (ret < 0) {
      LOG_ERROR("Failed to show bootloader status: %d", ret);
      continue;
    }

    ret = ali_psu_block_show(dev, ALI_PS_REG_MFR_ID, buf, &len);
    if (ret < 0) {
      LOG_ERROR("Failed to show MFR ID block: %d", ret);
      continue;
    }

    ret = ali_psu_block_hex_his_show(dev, ALI_PS_REG_MFR_POS_TOTAL, buf, &len);
    if (ret < 0) {
      LOG_ERROR("Failed to show block hex MFR_POS_TOTAL history: %d", ret);
      continue;
    }

    ret = ali_psu_sensor_show(dev, ALI_PS_REG_CURR_SHARE_IOUT_READ_FILTER, &val);
    if (ret < 0) {
      LOG_ERROR("Failed to show current sensor IOUT_READ_FILTER: %d", ret);
      continue;
    }
    ...
    if (ret < 0) {
      LOG_ERROR("Failed to get power: %d", ret);
      continue;
    }
    ...
    if (ret < 0) {
      LOG_ERROR("Failed to show byte history STATUS_INPUT: %d", ret);
      continue;
    }

    ret = ali_psu_word_his_show(dev, ALI_PS_REG_STATUS_WORD, buf);
    if (ret < 0) {
      LOG_ERROR("Failed to show word history STATUS_WORD: %d", ret);
      continue;
    }

    ret = ali_psu_word_show(dev, ALI_PS_REG_AUTO_TURN_ON_DELAY_WORD, &word);
    if (ret < 0) {
      LOG_ERROR("Failed to show word AUTO_TURN_ON_DELAY_WORD: %d", ret);
      continue;
    }

    ret = ali_psu_word_store(dev, ALI_PS_REG_AUTO_TURN_ON_DELAY_WORD, word);
    if (ret < 0) {
      LOG_ERROR("Failed to store word AUTO_TURN_ON_DELAY_WORD: %d", ret);
      continue;
    }

    ret = ali_psu_byte_show(dev, ALI_PS_REG_STATUS_OTHER, &buf[0]);
    if (ret < 0) {
      LOG_ERROR("Failed to show byte STATUS_OTHER: %d", ret);
      continue;
    }

    ret = ali_psu_byte_store(dev, ALI_PS_REG_STATUS_OTHER, buf[0]);
    if (ret < 0) {
      LOG_ERROR("Failed to store byte STATUS_OTHER: %d", ret);
      continue;
    }
