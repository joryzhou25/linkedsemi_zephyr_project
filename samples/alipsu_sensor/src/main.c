...
void main(void) {
  ...
  if (NULL == dev1) {
    LOG_ERROR("Failed to get ALIPSU1 device binding");
    return;
  }

  if (!device_is_ready(dev1)) {
    LOG_ERROR("Device %s is not ready", dev1->name);
    return;
  }

  if (NULL == dev2) {
    LOG_ERROR("Failed to get ALIPSU2 device binding");
    return;
  }

  if (!device_is_ready(dev2)) {
    LOG_ERROR("Device %s is not ready", dev2->name);
    return;
  }
  ...
  if (ret < 0) {
    LOG_ERROR("Failed to store new page to alipsu1: %d", old_page+1);
  }
  ...
  while (retry < 1) {
    ...
    if (ret < 0) {
      LOG_ERROR("Failed to get input voltage: %d", ret);
    }
    ...
    ret = sensor_channel_get(dev1, SENSOR_CHAN_VOLTAGE, &voltage);
    if (ret < 0) {
      LOG_ERROR("Failed to get input voltage1: %d", ret);
    }
    ...
    ret = sensor_channel_get(dev1, SENSOR_CHAN_VOLTAGE, &voltage);
    if (ret < 0) {
      LOG_ERROR("Failed to get output voltage: %d", ret);
    }
    ...
    if (ret < 0) {
      LOG_ERROR("Failed to get input current: %d", ret);
    }
    ...
    ret = sensor_channel_get(dev1, SENSOR_CHAN_CURRENT, &current);
    if (ret < 0) {
      LOG_ERROR("Failed to get output current: %d", ret);
    }
    ...
    if (ret < 0) {
      LOG_ERROR("Failed to get input power: %d", ret);
    }
    ...
    ret = sensor_channel_get(dev1, SENSOR_CHAN_POWER, &power);
    if (ret < 0) {
      LOG_ERROR("Failed to get output power: %d", ret);
    }
    ...
    if (ret < 0) {
      LOG_ERROR("Failed to get fan1: %d", ret);
    }
    ...
    ret = sensor_channel_get(dev1, SENSOR_CHAN_RPM, &fan);
    if (ret < 0) {
      LOG_ERROR("Failed to get fan2: %d", ret);
    }
    ...
    for (type = STATUS_WORD; type < STATUS_VOUT; type++) {
      ...
      if (ret < 0) {
        LOG_ERROR("Failed to get status [%d]: %d", type, ret);
      }
  ...
  if (ret < 0) {
    LOG_ERROR("Failed to store new page to alipsu2: %d", old_page+1);
  }
